...
- **Tests ride with their PR.** There is no trailing test phase. A PR that adds
  ...
  approval → commit) runs *inside* each PR.
...
### PR 2 — `feat/user-store-methods`
- `DataStore` gains `findUserByProvider`, `insertUser`.
- `postgres-store.ts`, `json-file-store.ts` (`StoreData` gains `users`),
  `memory-store.ts` (an array).

Tests: `memory-store` and `json-file-store` — insert then find by
`(provider, providerAccountId)`; unknown provider id returns undefined.
...
- `postgres-store.ts` — JOIN for `listAccountsForUser`, keeping
  `ORDER BY accounts.name`; `sql.transaction([...])` for `insertAccountWithOwner`.
- `json-file-store.ts` — `StoreData` gains `members`; the write queue makes the
  two-write insert atomic.
- `memory-store.ts` — a second array.
...
1. **Fetch first**, then `git checkout -b feat/members-schema origin/main`.
   This plan file is untracked — it survives the branch switch. Commit it first
   if you want it in the branch's history.
2. **Read before coding:** `src/db/index.ts` (store selection),
   `src/db/data-store.ts` (the interface all three stores implement),
   `src/app/page.tsx` (the unscoped `listAccounts()` call this feature exists to
   kill), and `AGENTS.md` — this is Next.js 16 and the docs in
   `node_modules/next/dist/docs/` are authoritative over training data.
3. **Repo workflow is checkpoint-driven, per PR.** Production code first,
   committed on approval, then tests one at a time. No commits or pushes without
   explicit approval. See CLAUDE.md.
4. **Start with PR 1.** It is purely additive — two unread tables and three
   types — so it is the cheapest thing to get merged and deployed.

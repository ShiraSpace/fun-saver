...
- **Create-account feature — DONE (end-to-end):** the form matches `docs/superpowers/specs/mockup-avatar-picker.html` — title `צור חשבון` (h2, white), name as a white card with inline `שם:` label + controlled input (`textMuted`/`textStrong` theme colours), 4-col capped grid of 20 circular avatars (per-avatar bg from `src/lib/avatars.ts`, selected = white border + `0 0 0 4px primary` ring + hover lift), gradient-pill CTA `✓ יצירת חשבון` gated on name+avatar. Form is a flex `<form>` anchored to the top with one uniform gap (`SCREEN_LAYOUT.gap - 2`) between every element and from the top. Submit → `useCreateAccount` (native `fetch`) → `POST /api/accounts` → `AccountsStore.createAccount(input, today())` seeds the account **+ 3 default wallets** (`DEFAULT_WALLETS`, savings 0.15) → `router.push('/')` → dashboard. `NameField` extracted to its own component. Acceptance E2E `create-account.e2e.ts` GREEN for the full journey (empty state → fill → pick → submit → dashboard with hero + 3 zero wallets).
- **Test status:** Jest 78/78 · visual E2E 28/28 · acceptance E2E 2/2 · `tsc`/lint clean.
- **Next (owed):**

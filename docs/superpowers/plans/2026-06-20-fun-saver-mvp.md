...
- **TDD outside-in:** acceptance E2E `e2e/create-account.e2e.ts` covers the **full journey** — empty-state CTA → fill name → pick avatar → submit → dashboard (header name + hero + 3 zero wallet balances) — and is GREEN. Commit only on explicit approval.
...
- **E2E:** TypeScript via `tsx` + `node:test`. Per-component drivers (composition): `session.ts` (lifecycle + `click`/`hover`/`clickSelector`/`hoverSelector`/`type`/`box`/`computedStyle`/`styleOf`/`waitForStyle`/`exists`/`count`/`text`/`texts`/`hasVerticalScroll`) + `menu`/`header`/`empty-state`/`create-account`/`avatar-picker`/`dashboard` drivers; `use-driver.ts` returns `{ session, menu, header, emptyState, createAccount, avatarPicker, dashboard }` (one server+browser per file). Use `clickSelector`/`hoverSelector` to target one element among many sharing a test-id (e.g. an avatar option). Keep drivers lean — add methods only when a test needs them. `session` = browser mechanism; drivers = app DSL on top, hiding CSS specifics (test-ids stay in each component's `constants.ts`). Shared CSS test helpers in `e2e/support/css-color.ts` (`hexToRgb`, `gradientToRgb`).
...
- **Avatars:** `public/avatars/kid-01..20.svg` via `next/image fill` (`unoptimized`). Shared `src/lib/avatars.ts` holds `AVATARS` (id + per-avatar background) + `avatarSource(id)`; reused by `Header` and `AvatarPicker`.
- **Create-account form:** the existing `CreateAccount` component owns name state + `selectedAvatarId`; renders a flex `<form>` (top-anchored, one uniform gap `CREATE_ACCOUNT_LAYOUT.gap = SCREEN_LAYOUT.gap - 2`) of `Title` + extracted `NameField` (controlled white card) + `AvatarPicker` (capped 4-col grid) + gradient `ActionButton` submit, disabled until name+avatar. Submit → co-located `useCreateAccount` hook (native `fetch`, `cache: 'no-store'`) `POST /api/accounts` → `router.push('/')`. **No react-query.**
- **Domain:** `AccountsStore` holds a `DataStore`. `createAccount(input, asOf)` inserts the account **and seeds the 3 `DEFAULT_WALLETS`** (savings `0.15`, spending/goodDeeds `0`) opened on `asOf`; the thin `POST /api/accounts` route passes `today()`. DAL: `DataStore` port (`insertAccount`/`listAccounts`/`insertWallet`/`listWalletsByAccount`/`insertTransactions`/`listTransactionsByWallet`) + `StoreData`; adapters `InMemoryStore` (unit tests) + `JsonFileStore` (file-backed, bootstrap-on-read, `FUNSAVER_DATA_PATH`); `getStore()` memoized per path. `page.tsx` reads `getStore().listAccounts()` + `getWalletsForAccount` directly — **note:** service-layer `listActiveAccounts` (active-only filter) still owed.
...
- **Foundations — DONE:** DAL (`DataStore` port + `InMemoryStore`/`JsonFileStore`/`getStore`); pure domain (money, dates, derivations, interest); context-driven theme (`ThemeTokens` + `theme.typography`, Emotion `ThemeProvider`, `test-support/render.tsx`); shared `Screen` + `ActionButton`; `page.tsx` Router (Account ↔ CreateAccount ↔ EmptyState).
- **Create-account — DONE (full E2E):** empty-state CTA → name card + 4-col avatar picker → `useCreateAccount` `POST /api/accounts` (`createAccount` seeds the 3 default wallets) → dashboard. Matches `mockup-avatar-picker.html`.
- **App menu (static shell) — DONE (PR #1):** hamburger opens a fixed full-screen `MenuOverlay`; one morphing hamburger↔X toggle floats above it (white when open, Escape closes). Three non-interactive sections — Accounts (`AccountChip` avatar+letter chips + `Edit`/`AddAccountChip`), Appearance (3 swatches), Language (he/EN). Extracted shared `Avatar` (size + fill), `BurgerIcon`, `MenuLabel`; typography moved into `theme.typography`. Spec: `2026-06-21-app-menu-design.md`.
- **Next (owed):**
  1. Error/loading states for the submit (currently the create promise isn't surfaced to the user) + a `400` guard + test on `POST /api/accounts` for invalid input.
  2. Service layer `listActiveAccounts` (active-only filter) so `page.tsx` stops reading the raw store directly.
  3. Transaction flow (deposit/withdraw) + interest accrual on the dashboard.
  4. **Wire the menu interactions:** account switching, edit-account, theme switching, language toggle — plus a real i18n layer + additional registry themes; promote placeholder menu colours (badge/ring/swatch, `ActionChip` rgba) to theme tokens once a 2nd theme ships.
  5. **Menu polish:** the app action-FAB still floats above the open overlay (decide cover-vs-hide); `Header` avatar is now circular via shared `Avatar`; add overlay body-scroll-lock + focus-trap.

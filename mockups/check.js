...
function assertIndexLinksEveryPage(pages) {
  const index = fs.readFileSync(path.join(MOCKUPS, 'index.html'), 'utf8');
  const linked = new Set([...index.matchAll(/href="([^"]+\.html)"/g)]
    .map((match) => path.normalize(match[1])));

  const unlisted = pages
    .map(shortName)
    .filter((page) => page !== 'index.html' && !linked.has(path.normalize(page)));

  assert.deepStrictEqual(unlisted, [],
    `index.html לא מזכיר: ${unlisted.join(', ')} — כל מוקאפ חייב שורה עם ההחלטה שלו`);
}
...
assertIndexLinksEveryPage(allPages);
console.log(`ok   index.html מזכיר את כל ${allPages.length - 1} המוקאפים`);

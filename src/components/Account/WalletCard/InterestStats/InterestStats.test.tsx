...
describe('InterestStats', () => {
  describe('when the daily interest rounds up', () => {
    ...
    it('shows the interest earned so far in whole shekels', () => {
      expect(
        screen.getByTestId(INTEREST_STATS_TEST_IDS.interestGain)
      ).toHaveTextContent('₪67');
    });

    it('shows the daily interest', () => {
      expect(
        screen.getByTestId(INTEREST_STATS_TEST_IDS.todayInterest)
      ).toHaveTextContent('₪1');
    });
    ...
    it('labels the interest cell', () => {
      expect(
        screen.getByTestId(INTEREST_STATS_TEST_IDS.stats)
      ).toHaveTextContent(INTEREST_STATS_COPY.interestGainLabel);
    });

    it('paints the interest label with the gain token', () => {
      const label = screen.getByText(INTEREST_STATS_COPY.interestGainLabel);
    ...
    it('labels the daily cell', () => {
      expect(
        screen.getByTestId(INTEREST_STATS_TEST_IDS.stats)
      ).toHaveTextContent(INTEREST_STATS_COPY.todayInterestLabel);
    });
  });
  ...
  it('hides the daily interest when it rounds to zero', () => {
    render(
      <InterestStats principal={13700} interestGain={6743} todayInterest={20} />
    );

    expect(
      screen.queryByTestId(INTEREST_STATS_TEST_IDS.todayInterest)

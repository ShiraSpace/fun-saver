import { render, screen } from '@/test-utils/render';
import { SOURCES_SECTION_ID } from '../constants';
import { MethodSection } from './MethodSection';
import { METHOD_SECTION_TEST_IDS } from './constants';

describe('a method section', () => {
  const mockNumber = 1;
  const mockTitle = 'למה לא קופה אחת';
  const mockBody = 'גוף הסעיף';

  describe('as the page first renders it', () => {
    beforeEach(() => {
      render(
        <MethodSection id={mockNumber} title={mockTitle}>
          {mockBody}
        </MethodSection>
      );
    });

    it('starts closed, so six sections read as a list rather than a wall', () => {
      expect(
        screen.getByTestId(METHOD_SECTION_TEST_IDS.section(mockNumber))
      ).not.toHaveAttribute('open');
    });

    it('shows what the section has to say once the parent opens it', () => {
      expect(
        screen.getByTestId(METHOD_SECTION_TEST_IDS.body(mockNumber))
      ).toHaveTextContent(mockBody);
    });

    it('says nothing extra on the row when there is nothing extra to say', () => {
      expect(
        screen.queryByTestId(METHOD_SECTION_TEST_IDS.hint(mockNumber))
      ).not.toBeInTheDocument();
    });
  });

  describe('when the section carries a hint', () => {
    const mockHint = 'הכי חשוב';

    beforeEach(() => {
      render(
        <MethodSection id={mockNumber} title={mockTitle} hint={mockHint}>
          {mockBody}
        </MethodSection>
      );
    });

    it('puts it on the row itself, the only thing a shut section can tell the parent', () => {
      expect(
        screen.getByTestId(METHOD_SECTION_TEST_IDS.hint(mockNumber))
      ).toHaveTextContent(mockHint);
    });
  });

  describe('when the section is not one of the numbered steps', () => {
    beforeEach(() => {
      render(
        <MethodSection id={SOURCES_SECTION_ID} title={mockTitle}>
          {mockBody}
        </MethodSection>
      );
    });

    it('shows no number, so the six steps a parent has to follow stay six', () => {
      expect(
        screen.queryByTestId(
          METHOD_SECTION_TEST_IDS.numeral(SOURCES_SECTION_ID)
        )
      ).not.toBeInTheDocument();
    });

    it('still shows what it holds, so it is not a heading with nothing under it', () => {
      expect(
        screen.getByTestId(METHOD_SECTION_TEST_IDS.body(SOURCES_SECTION_ID))
      ).toHaveTextContent(mockBody);
    });
  });
});

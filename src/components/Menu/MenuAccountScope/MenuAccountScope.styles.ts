...
import { MENU_ACCOUNT_SCOPE_STYLE } from './constants';
...
export const AccountBlock = styled(ScopeBlock)`
  ...
  box-shadow: inset ${MENU_ACCOUNT_SCOPE_STYLE.stripeWidth}px 0 0 ${stripe};
`;

export const Head = styled.h2`
  display: flex;
  align-items: center;
  gap: ${MENU_ACCOUNT_SCOPE_STYLE.headGap}px;
  margin: 0 0 ${MENU_ACCOUNT_SCOPE_STYLE.headMarginBottom}px;
  font-size: ${headingSize}px;
  font-weight: 700;
`;

export const Note = styled.p`
  margin: 0 0 ${MENU_ACCOUNT_SCOPE_STYLE.noteMarginBottom}px;
  text-align: start;
  font-size: ${noteSize}px;
  line-height: ${MENU_ACCOUNT_SCOPE_STYLE.noteLineHeight};
  color: ${mutedText};
`;
